...
OUTPUT_DIR = ROOT / "literature" / "Lawyers and Courts"
FULL_MANIFEST = OUTPUT_DIR / "lawyers_courts_download_manifest.csv"
MISSING_MANIFEST = OUTPUT_DIR / "lawyers_courts_undownloadable_metadata.csv"
...
SOURCES = [
    ...
    # OpenAlex currently indexes this title as "American Economic Review Insights".
    Source(
        "AEJINSIGHTS",
...
TITLE_MATCH_RULES = {
    ...
    "lawyer": re.compile(r"\blawyers?\b", re.IGNORECASE),
    "judicial": re.compile(r"\bjudicial\b", re.IGNORECASE),
    "legal": re.compile(r"\blegal\b", re.IGNORECASE),
    "legal system": re.compile(r"\blegal system\b", re.IGNORECASE),
    "rule of law": re.compile(r"\brule of (?:the )?law\b", re.IGNORECASE),
    "law": re.compile(r"\blaws?\b", re.IGNORECASE),
}

LAW_CONTEXT_RULE = re.compile(
    r"\b("
    r"constitution(?:al)?|rights?|judge|judges|justice|court|courts|"
    r"legal|lawyer|lawyers|judicial|attorney|attorneys|"
    r"enforcement|litigation|rule"
    r")\b",
    re.IGNORECASE,
)
...
NON_TARGET_LAW_RULES = [
    re.compile(r"\binternational law\b", re.IGNORECASE),
    re.compile(r"\bdiplomac(?:y|ic)\b", re.IGNORECASE),
]
...
def evaluate_match(item: dict) -> tuple[set[str], set[str]]:
    ...
    if first_page and last_page and first_page.isdigit() and last_page.isdigit():
        if int(last_page) - int(first_page) + 1 <= 3:
            return set(), set()

    matched_terms = {
        label for label, pattern in TITLE_MATCH_RULES.items() if pattern.search(title)
    }
    if not matched_terms:
        return set(), set()

    if any(pattern.search(title) for pattern in NON_TARGET_LAW_RULES):
        if not re.search(r"\b(court|courts|judicial|judge|judges|justice|tribunal)\b", title, re.IGNORECASE):
            return set(), set()

    if matched_terms == {"law"} and not (
        TITLE_MATCH_RULES["rule of law"].search(title)
        or LAW_CONTEXT_RULE.search(title)
        or re.search(r"\blaw and\b", title, re.IGNORECASE)
    ):
        return set(), set()

    return matched_terms, {"title"}
        ...
        for term in SEARCH_TERMS:
            ...
            while True:
                filter_value = (
                    ...
                    f"from_publication_date:{YEAR_CUTOFF}-01-01,"
                ...
                for item in results:
                    ...
                    matched_terms, matched_fields = evaluate_match(item)
                    if not matched_terms:
                        continue
            ...
            print(f"  - {term}: {seen_this_query} candidate hits")
            time.sleep(0.15)
...
def download_pdf(session: requests.Session, item: dict) -> tuple[Path | None, str]:
...
def download_worker(index: int, total: int, item: dict) -> tuple[int, dict, Path | None, str]:
    path, note = download_pdf(worker_session(), item)
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=DOWNLOAD_WORKERS) as executor:
        futures = [
            executor.submit(download_worker, index, total, item)
    ...
    write_csv(FULL_MANIFEST, all_rows)
    write_csv(MISSING_MANIFEST, missing_rows)
    ...
    print(f"[summary] manifest: {FULL_MANIFEST}")
    print(f"[summary] undownloadable metadata: {MISSING_MANIFEST}")
